"""
`FakeRavencoinProxy` allows for unit testing of code that normally uses ravencoin
RPC without requiring a running ravencoin node.

`FakeRavencoinProxy` has an interface similar to `ravencoin.rpc.Proxy`, but does not
connect to a local ravencoin RPC node. Hence, `FakeRavencoinProxy` is similar to a
mock for the RPC tests.

`FakeRavencoinProxy` does _not_ implement a full ravencoin RPC node. Instead, it
...
from ravencoin.core import (
    ...
    # number of satoshis per ravencoin
    COIN,
...
from ravencoin.wallet import (
    # ravencoin address initialized from base58-encoded string
    CRavencoinAddress,
...
def make_address_from_passphrase(passphrase, compressed=True, as_str=True):
    ...
    private_key = CRavencoinSecret.from_secret_bytes(passphrasehash, compressed=compressed)
...
class FakeRavencoinProxy(object):
    """
    This is an alternative to using `ravencoin.rpc.Proxy` in tests. This class
    ...
    def _call(self, rpc_method_name, *args, **kwargs):
        ...
        python-ravencoinlib does not parse.
        """
    ...
    def getblock(self, blockhash, *args, **kwargs):
        ...
        # Note that the actual "getblock" ravencoind RPC call from
        # python-ravencoinlib returns a CBlock object, not a dictionary.
    ...
    def _batch(self, batch_request_entries):
        ...
        feature found in python-ravencoinlib and ravencoind RPC.
        """
        ...
        for (idx, request) in enumerate(batch_request_entries):
            ...
            for necessary_key in necessary_keys:
                if not necessary_key in request.keys():
                    raise FakeRavencoinProxyException("Missing necessary key {} for _batch request number {}".format(necessary_key, idx))

            if isinstance(request["params"], list):
                method = getattr(self, request["method"])
                result = method(*request["params"])
            else:
                # matches error message received through python-ravencoinrpc
