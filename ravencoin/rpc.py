...
import ravencoin
from ravencoin.core import COIN, x, lx, b2lx, CBlock, CBlockHeader, CTransaction, COutPoint, CTxOut
from ravencoin.core.script import CScript
...
class BaseProxy(object):
    ...
    def __init__(self,
        ...
        if service_url is None:
            # Figure out the path to the ravencoin.conf file
            if btc_conf_file is None:
                if platform.system() == 'Darwin':
                    btc_conf_file = os.path.expanduser('~/Library/Application Support/Ravencoin/')
                elif platform.system() == 'Windows':
                    btc_conf_file = os.path.join(os.environ['APPDATA'], 'Ravencoin')
                else:
                    btc_conf_file = os.path.expanduser('~/.ravencoin')
                btc_conf_file = os.path.join(btc_conf_file, 'ravencoin.conf')

            # Ravencoin Core accepts empty rpcuser, not specified in btc_conf_file
            conf = {'rpcuser': ""}

            # Extract contents of ravencoin.conf to build service_url
            try:
            ...
            # Treat a missing ravencoin.conf as though it were empty
            except FileNotFoundError:
                pass

            if service_port is None:
                service_port = ravencoin.params.RPC_PORT
            ...
            if ravencoin.params.NAME != "mainnet":
                cookie_dir = os.path.join(cookie_dir, ravencoin.params.NAME)
...
class RawProxy(BaseProxy):
    """Low-level proxy to a ravencoin JSON-RPC service
    ...
    def __getattr__(self, name):
        ...
        # Make debuggers show <function ravencoin.rpc.name> rather than <function
        # ravencoin.rpc.<lambda>>
        f.__name__ = name
        return f


class Proxy(BaseProxy):
    """Proxy to a ravencoin RPC service

    Unlike ``RawProxy``, data is passed as ``ravencoin.core`` objects or packed
    ...
    def __init__(self,
        ...
        specified, ``~/.ravencoin/ravencoin.conf`` or equivalent is used by
        default.  The default port is set according to the chain parameters in
        use: mainnet, testnet, or regtest.

        Usually no arguments to ``Proxy()`` are needed; the local ravencoind will
        be used.
    ...
    def submitblock(self, block, params=None):
        ...
        params is optional and is currently ignored by ravencoind. See
        https://en.ravencoin.it/wiki/BIP_0022 for full specification.
        """
